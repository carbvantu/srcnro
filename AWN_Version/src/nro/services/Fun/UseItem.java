...
public class UseItem {
            ...
            if (item.template.strRequire <= pl.nPoint.power) {
                switch (item.template.type) {
                    ...
                    case 7:
                        ...
                        break;
                    case 36:
                    case 72: // Danh hieu (Badges)
                        useBadgeItem(pl, item);
                        break;
                    case 33:
                        UseCard(pl, item);
                        break;
                    ...
                    case 39:
                        ...
                        break;
                    case 35:
                        if (isSpecialSkillBook(item)) {
                            learnSkillFromBook(pl, item);
                        } else {
                            InventoryService.gI().itemBagToBody(pl, indexBag);
                        }
                        break;
    ...
    private boolean isSpecialSkillBook(Item item) {
        if (item == null || item.template == null) return false;
        int id = item.template.id;
        return (id == 1044 || id == 1211 || id == 1212 || id == 1278 || id == 1279 || id == 1280);
    }

    private byte getSkillIdFromBook(Item item) {
        if (item == null || item.template == null) return -1;
        int id = item.template.id;
        switch (id) {
            case 1341: // Sách Super Kamejoko
            case 1044: // Sách tuyệt kỹ 1 (Trái Đất)
            case 1278: // Sách tuyệt kỹ 2 (Trái Đất)
                return Skill.SUPER_KAME;
            case 1342: // Sách Ma phong ba
            case 1211: // Sách tuyệt kỹ 1 (Namếc)
            case 1279: // Sách tuyệt kỹ 2 (Namếc)
                return Skill.MA_PHONG_BA;
            case 1343: // Sách Cađíc liên hoàn chưởng
            case 1212: // Sách tuyệt kỹ 1 (Xayda)
            case 1280: // Sách tuyệt kỹ 2 (Xayda)
                return Skill.LIEN_HOAN_CHUONG;
            case 1319: return Skill.DRAGON;
            case 1320: return Skill.KAMEJOKO;
            case 1321: return Skill.THAI_DUONG_HA_SAN;
            case 1322: return Skill.KAIOKEN;
            case 1323: return Skill.QUA_CAU_KENH_KHI;
            case 1324: return Skill.DICH_CHUYEN_TUC_THOI;
            case 1325: return Skill.THOI_MIEN;
            case 1326: return Skill.DEMON;
            case 1327: return Skill.MASENKO;
            case 1328: return Skill.TRI_THUONG;
            case 1329: return Skill.MAKANKOSAPPO;
            case 1330: return Skill.DE_TRUNG;
            case 1331: return Skill.SOCOLA;
            case 1332: return Skill.LIEN_HOAN;
            case 1333: return Skill.GALICK;
            case 1334: return Skill.ANTOMIC;
            case 1335: return Skill.TAI_TAO_NANG_LUONG;
            case 1336: return Skill.BIEN_KHI;
            case 1337: return Skill.TU_SAT;
            case 1338: return Skill.TROI;
            case 1339: return Skill.HUYT_SAO;
            case 1340: return Skill.KHIEN_NANG_LUONG;
            default:
                return -1;
        }
    }

    private void learnSkillFromBook(Player pl, Item item) {
        if (pl == null || item == null || item.template == null) return;
        byte skillId = getSkillIdFromBook(item);
        if (skillId == -1) {
            Service.gI().sendThongBao(pl, "Vật phẩm không phải sách kỹ năng hợp lệ!");
            return;
        }
        if (item.template.gender < 3 && item.template.gender != pl.gender) {
            Service.gI().sendThongBao(pl, "Hành tinh của bạn không phù hợp để học kỹ năng này!");
            return;
        }
        Skill curSkill = pl.playerSkill.getSkillbyId(skillId);
        int curPoint = (curSkill != null) ? curSkill.point : 0;
        if (curPoint >= 7) {
            Service.gI().sendThongBao(pl, "Kỹ năng đã đạt cấp tối đa (cấp 7)!");
            return;
        }
        int nextPoint = curPoint + 1;
        Skill newSkill = SkillUtil.createSkill(skillId, nextPoint);
        ...
        try {
            if (curPoint == 0) {
                ...
                pl.sendMessage(msg);
                Service.gI().sendThongBao(pl, "Chúc mừng! Bạn đã học thành công kỹ năng " + newSkill.template.name + "!");
            } else {
                msg = Service.gI().messageSubCommand((byte) 62);
                msg.writer().writeShort(newSkill.skillId);
                pl.sendMessage(msg);
                Service.gI().sendThongBao(pl, "Kỹ năng " + newSkill.template.name + " đã được nâng lên cấp " + nextPoint + "!");
            }
        } catch (Exception e) {
            Logger.logException(UseItem.class, e);
        } finally {
        ...
        InventoryService.gI().sendItemBag(pl);
        jbcd.dao.PlayerDAO.updatePlayer(pl);
    }

    private void useBadgeItem(Player pl, Item item) {
        if (pl == null || item == null) return;
        int badgeEffectId = 257; // Default: ID 257 is ADMIN - QUAN TRI VIEN
        if (item.template != null) {
            int eff = BagesTemplate.fineIdEffectbyIdItem(item.template.id);
            if (eff > 0) {
                badgeEffectId = eff;
            }
        }
        ...
        if (pl.dataBadges != null) {
            for (BadgesData bd : pl.dataBadges) {
                if (bd.idBadGes == badgeEffectId) {
                    found = true;
                    bd.isUse = !bd.isUse;
                    if (bd.isUse) {
                        BadgesService.turnOnBadges(pl, badgeEffectId);
                        if (pl.badges != null) pl.badges.idBadges = badgeEffectId;
                        Service.gI().sendBadgesPlayer(pl, 5, badgeEffectId);
                        Service.gI().sendThongBao(pl, "Đã kích hoạt danh hiệu: " + item.template.name);
                    } else {
                        if (pl.badges != null) pl.badges.idBadges = -1;
                        Service.gI().sendBadgesPlayer(pl, 5, -1);
                        Service.gI().sendThongBao(pl, "Đã tháo gỡ danh hiệu: " + item.template.name);
                    }
                    break;
                }
            }
            if (!found) {
                BadgesData bd = new BadgesData(pl, badgeEffectId, 365);
                BadgesService.turnOnBadges(pl, badgeEffectId);
                if (pl.badges != null) pl.badges.idBadges = badgeEffectId;
                Service.gI().sendBadgesPlayer(pl, 5, badgeEffectId);
                Service.gI().sendThongBao(pl, "Đã kích hoạt danh hiệu: " + item.template.name);
