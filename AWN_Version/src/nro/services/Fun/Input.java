            ...
            switch (player.iDMark.getTypeInput()) {
                ...
                case PASS_ADMIN: {
                    if (text.length == 0 || text[0] == null) {
                        Service.gI().sendThongBao(player, "Vui lòng nhập mật khẩu Admin!");
                        return;
                    }
                    ...
                    if (matKhauNhap.equals("admin123")) {
                        ...
                        Logger.warning("Player " + player.name + " vừa truy cập vào Menu Admin thành công\n");
                        Service.gI().sendThongBao(player, "Đăng nhập Quyền Quản Trị Hệ Thống thành công!");
                        Service.gI().OpenMenuKeyOrAdmin(player);
                    } else {
                        Service.gI().sendThongBao(player, "Mật khẩu Admin không chính xác! (Mật khẩu mặc định: admin123)");
                        Logger.warning("Player " + player.name + " nhập sai mật khẩu Admin: " + matKhauNhap + "\n");
                    }
                    break;
                }
                case DICH_CHUYEN_MAP: {
                    try {
                        int mapId = Integer.parseInt(text[0].trim());
                        ChangeMapService.gI().changeMapPlayer(player, mapId, -1, 300, 300);
                        Service.gI().sendThongBao(player, "Đã dịch chuyển đến map " + mapId + "!");
                    } catch (Exception ex) {
                        Service.gI().sendThongBao(player, "ID Map không hợp lệ hoặc không tìm thấy map!");
                    }
                    break;
                }
...
//---------------------------------------------------------------------------------------                    
    ...
    public void createFormDichChuyenMap(Player pl) {
        createForm(pl, DICH_CHUYEN_MAP, "Dịch Chuyển Đến Map Bất Kỳ", new SubInput("Nhập ID Map (0 - 248)", NUMERIC));
    }
