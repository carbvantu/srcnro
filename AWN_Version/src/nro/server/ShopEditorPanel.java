...
public class ShopEditorPanel extends JPanel {
    ...
    private void initUI() {
        ...
        JButton btnQuickAdd = ServerGuiUtils.createStyledButton("+ Thêm Item Vào Shop NPC", new Color(220, 53, 69), Color.WHITE);
        btnQuickAdd.addActionListener(e -> openQuickAddItemDialog());
            ...
            new Thread(() -> {
                try (Connection conn = getConnection()) {
                    ...
                    SwingUtilities.invokeLater(()
                            -> JOptionPane.showMessageDialog(this, "Đã lưu và cập nhật Shop thành công!")
                    );
    ...
    private static class ShopComboItem {
        int shopId;
        int npcId;
        String name;
        public ShopComboItem(int shopId, int npcId, String name) {
            this.shopId = shopId;
            this.npcId = npcId;
            this.name = name;
        }
        ...
        public String toString() {
            return "[Shop " + shopId + "] NPC: " + name + " (ID " + npcId + ")";
        }
    }

    private static class TabComboItem {
        int tabId;
        String name;
        public TabComboItem(int tabId, String name) {
            this.tabId = tabId;
            this.name = name;
        }
        @Override
        public String toString() {
            return "[Tab " + tabId + "] " + name;
        }
    }

    public void openQuickAddItemDialog() {
        JDialog dialog = new JDialog((Frame) SwingUtilities.getWindowAncestor(this), "Thêm Nhanh Vật Phẩm Vào Shop NPC", true);
        dialog.setSize(650, 680);
        dialog.setLocationRelativeTo(this);
        dialog.setLayout(new BorderLayout(10, 10));
        ...
        JComboBox<ShopComboItem> cbShops = new JComboBox<>();
        JComboBox<TabComboItem> cbTabs = new JComboBox<>();

        try (Connection conn = getConnection(); Statement st = conn.createStatement();
             ResultSet rs = st.executeQuery("SELECT s.id, s.npc_id, n.name FROM shop s LEFT JOIN npc_template n ON s.npc_id = n.id ORDER BY s.npc_id ASC, s.id ASC")) {
            while (rs.next()) {
                int sId = rs.getInt("id");
                int nId = rs.getInt("npc_id");
                String nName = rs.getString("name");
                if (nName == null) nName = "Chưa rõ";
                cbShops.addItem(new ShopComboItem(sId, nId, nName));
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
        ...
        Runnable updateTabs = () -> {
            ...
            ShopComboItem selected = (ShopComboItem) cbShops.getSelectedItem();
            if (selected != null) {
                try (Connection conn = getConnection(); PreparedStatement ps = conn.prepareStatement("SELECT id, name FROM tab_shop WHERE shop_id = ? ORDER BY id ASC")) {
                    ps.setInt(1, selected.shopId);
                    try (ResultSet rs = ps.executeQuery()) {
                        while (rs.next()) {
                            cbTabs.addItem(new TabComboItem(rs.getInt("id"), rs.getString("name")));
                        }
                    }
                } catch (Exception ex) {
                    ex.printStackTrace();
        ...
        cbShops.addActionListener(e -> updateTabs.run());
        updateTabs.run();

        JLabel lblSelectedItem = new JLabel("Chưa chọn vật phẩm");
        lblSelectedItem.setFont(new Font("Tahoma", Font.BOLD, 12));
        lblSelectedItem.setForeground(new Color(0, 102, 204));
        final int[] selectedItemId = {-1};

        JButton btnChooseItem = ServerGuiUtils.createStyledButton("Chọn Item...", new Color(0, 123, 255), Color.WHITE);
        btnChooseItem.addActionListener(e -> {
            showSearchDialog(dialog, "Chọn Vật Phẩm", itemTemplateMap, (itemId, itemName) -> {
                selectedItemId[0] = itemId;
                lblSelectedItem.setText(itemName + " (ID: " + itemId + ")");
                lblSelectedItem.setIcon(getItemIcon(itemId));
            });
        });

        JTextField txtCost = new JTextField("1000", 10);
        JComboBox<String> cbTypeSell = new JComboBox<>(new String[]{
                "0 - Vàng (Gold)",
                "1 - Ngọc Xanh (Gem)",
                "2 - Hồng Ngọc (Ruby)"
        });

        JCheckBox chkVipStats = new JCheckBox("Thêm chỉ số mặc định (Sức đánh +30%, HP +30%, KI +30%)", true);
        chkVipStats.setFont(new Font("Tahoma", Font.PLAIN, 12));

        gbc.gridx = 0; gbc.gridy = 0;
        formPanel.add(new JLabel("1. Chọn NPC / Shop:"), gbc);
        gbc.gridx = 1;
        formPanel.add(cbShops, gbc);

        gbc.gridx = 0; gbc.gridy = 1;
        formPanel.add(new JLabel("2. Chọn Tab Shop:"), gbc);
        gbc.gridx = 1;
        formPanel.add(cbTabs, gbc);

        gbc.gridx = 0; gbc.gridy = 2;
        formPanel.add(new JLabel("3. Vật phẩm:"), gbc);
        gbc.gridx = 1;
        JPanel itemChoosePanel = new JPanel(new FlowLayout(FlowLayout.LEFT, 5, 0));
        itemChoosePanel.setOpaque(false);
        itemChoosePanel.add(btnChooseItem);
        itemChoosePanel.add(lblSelectedItem);
        formPanel.add(itemChoosePanel, gbc);

        gbc.gridx = 0; gbc.gridy = 3;
        formPanel.add(new JLabel("4. Giá bán:"), gbc);
        gbc.gridx = 1;
        formPanel.add(txtCost, gbc);

        gbc.gridx = 0; gbc.gridy = 4;
        formPanel.add(new JLabel("5. Loại tiền:"), gbc);
        gbc.gridx = 1;
        formPanel.add(cbTypeSell, gbc);

        gbc.gridx = 0; gbc.gridy = 5; gbc.gridwidth = 2;
        formPanel.add(chkVipStats, gbc);

        dialog.add(formPanel, BorderLayout.CENTER);

        JPanel bottom = new JPanel(new FlowLayout(FlowLayout.RIGHT, 10, 10));
        JButton btnAdd = ServerGuiUtils.createStyledButton("Thêm Vào Shop Ngay", new Color(40, 167, 69), Color.WHITE);
        JButton btnCancel = ServerGuiUtils.createStyledButton("Đóng", new Color(108, 117, 125), Color.WHITE);
        btnCancel.addActionListener(e -> dialog.dispose());

        btnAdd.addActionListener(e -> {
            if (selectedItemId[0] == -1) {
                JOptionPane.showMessageDialog(dialog, "Vui lòng chọn vật phẩm cần thêm!", "Cảnh báo", JOptionPane.WARNING_MESSAGE);
                return;
            }
            TabComboItem selectedTab = (TabComboItem) cbTabs.getSelectedItem();
            if (selectedTab == null) {
                JOptionPane.showMessageDialog(dialog, "Vui lòng chọn Tab shop!", "Cảnh báo", JOptionPane.WARNING_MESSAGE);
                return;
            }
            ...
            try {
                ...
                JOptionPane.showMessageDialog(dialog, "Giá bán không hợp lệ!", "Lỗi", JOptionPane.ERROR_MESSAGE);
                return;
            }
            int typeSell = cbTypeSell.getSelectedIndex();

            try (Connection conn = getConnection()) {
                conn.setAutoCommit(false);
                int itemShopId = -1;
                try (PreparedStatement ps = conn.prepareStatement(
                        "INSERT INTO item_shop (tab_id, temp_id, is_new, is_sell, type_sell, cost, icon_spec) VALUES (?, ?, 1, 1, ?, ?, 0)",
                        Statement.RETURN_GENERATED_KEYS)) {
                    ps.setInt(1, selectedTab.tabId);
                    ps.setInt(2, selectedItemId[0]);
                    ps.setInt(3, typeSell);
                    ps.setInt(4, cost);
                    ps.executeUpdate();
                    try (ResultSet gk = ps.getGeneratedKeys()) {
                        if (gk.next()) itemShopId = gk.getInt(1);
                    }
                }

                if (chkVipStats.isSelected() && itemShopId != -1) {
                    try (PreparedStatement psOpt = conn.prepareStatement("INSERT INTO item_shop_option (item_shop_id, option_id, param) VALUES (?, ?, ?)")) {
                        psOpt.setInt(1, itemShopId); psOpt.setInt(2, 50); psOpt.setInt(3, 30); psOpt.executeUpdate();
                        psOpt.setInt(1, itemShopId); psOpt.setInt(2, 77); psOpt.setInt(3, 30); psOpt.executeUpdate();
                        psOpt.setInt(1, itemShopId); psOpt.setInt(2, 103); psOpt.setInt(3, 30); psOpt.executeUpdate();
                    }
                }
                ...
                JOptionPane.showMessageDialog(dialog, "Đã thêm vật phẩm vào Shop thành công!\nNPC đang bán vật phẩm này ngay lập tức.", "Thành công", JOptionPane.INFORMATION_MESSAGE);
                dialog.dispose();
            } catch (Exception ex) {
                ex.printStackTrace();
                JOptionPane.showMessageDialog(dialog, "Lỗi thêm vật phẩm vào shop: " + ex.getMessage(), "Lỗi", JOptionPane.ERROR_MESSAGE);
            }
        });
